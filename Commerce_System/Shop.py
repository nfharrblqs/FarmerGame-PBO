...
class Shop(GameObject):
    ...
    def buy(self, player, item_name):
        ...
        if item_name in self.shopItems:
            ...
            return player.buyItem(item_name, price)
    ...
    def sell_item(self, player, item_name):
        ...
        if player._inventory.hasItem(item_name):

            sell_price = self.shopItems.get(item_name, 0) // 2
            player._money += sell_price

            player._inventory.removeItem(item_name)
    ...
    def buy_animal(self, player, animal_type, game):
        ...
        if animal_type in self.animalItems:
            ...
            if player._money >= price:
                player._money -= price
                
                player._inventory.addItem(animal_type)
                
                print(f"{animal_type} bought for {price} money!")
                return True
            else:
                print(f"Not enough money! Need {price} money")
                return False
        else:
    ...
    def sell_animal(self, player, animal_obj, game):
        ...
        if animal_type in self.animalSellPrices:
            ...
            print(f"{animal_type} sold for {sell_price} money!")
            return True
        else:
